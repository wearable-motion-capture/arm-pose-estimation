...
def est_to_ori_cal_larm_uarm_hips(est: np.array, body_measure: np.array):
    ...
    if est.shape[0] > 1:
        ...
        p_uarm_orig_g = ts.quat_rotate_vector(p_hips_quat_g, BoneMap.DEFAULT_UARM_ORIG_RH)
...
def est_to_ori_cal_larm_uarm(est: np.array, body_measure: np.array):
    ...
    if est.shape[0] > 1:
        ...
        # get the transition from upper arm origin to lower arm origin
        pred_larm_origin_rh = ts.quat_rotate_vector(pred_uarm_rot_rh, uarm_vec) + uarm_orig
    ...
    return np.hstack([
        pred_larm_rot_rh,  # hand rot [0,1,2,3]
        pred_hand_origin_rh,  # hand orig [4,5,6]
        pred_larm_rot_rh,  # larm rot [7,8,9,10]
        pred_larm_origin_rh,  # larm orig [11,12,13]
        pred_uarm_rot_rh,  # uarm rot [14,15,16,17]
